...
class LinesAnalizator():
    def __init__(self, predictor, accamulators_length=10):
        ...
        for line in batch_lines[0]:
            ...
            for point in line.points:
                if abs(point[1] - y) < min_distance:
                    cv2.circle(image, (int(point[0]), y), 3, (0, 0, 255), -1)
    ...
    def get_current_car_line(self, image, line_points, left_curbside, right_curbside, image_size_x, car_point_part=0.5):
    ...
    def get_count_roads(self, image, batch_lines, min_y, max_y, image_size_x, gap=20, min_distance=20, car_point_part=0.5):
        ...
        for line_id in range(count_lines):
            line_y = max_y - line_id * gap

            cv2.line(image, (0, line_y), (image.shape[1], line_y), (0, 255, 0), 2)
            ...
            car_line_id = self.get_current_car_line(image, line_points, left_curbside, right_curbside, image_size_x, car_point_part)
    ...
    def analize_roads_with_accamulator(self, image, batch_lines, min_y, max_y, image_size_x, gap=20, min_distance=20, car_point_part=0.5, reset_accamulator=False):
        count_roads, car_line_id = self.get_count_roads(image, batch_lines, min_y, max_y, image_size_x, gap, min_distance, car_point_part)
