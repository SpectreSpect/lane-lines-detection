...
def from_mask_batches_to_yolo(mask_batches: list, output_path: str):
    for idx, masks in enumerate(mask_batches):
        file_path = os.path.join(output_path, f"frame_{idx:05d}.txt")
        from_masks_to_yolo(masks, file_path)


def from_xml_to_yolo(xml_file_path: str, output_path: str, label_names: str, images_shape=(1920, 1080)):
    ...
    from_mask_batches_to_yolo(lane_mask_batches, output_path)
...
def from_cvat_to_yolo(data_folder: str, output_images_folder: str, output_labels_folder: str, label_names: list):
    ...
    from_xml_to_yolo(xml_path, output_labels_folder, label_names, shape)
